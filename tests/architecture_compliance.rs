...
fn without_a_terminal_the_default_downloads_and_an_explicit_ask_refuses() {
    ...
    fixture.file("runner.toml", "download = \"ask\"\n");
    let asked = install();
    assert!(!asked.status.success());
    assert!(
        String::from_utf8_lossy(&asked.stderr).contains("refused"),
        "{}",
        String::from_utf8_lossy(&asked.stderr)
    );
    fixture.assert_not_executed();

    std::fs::remove_file(fixture.0.join("runner.toml")).unwrap();
...
fn explain_never_executes_sequential_or_parallel_chains() {
    for mode in ["-s", "-p"] {
        let fixture = Fixture::new();
        let output = fixture.run(&["--dry-run", "run", mode, "first", "second"], "true");
        assert!(
            output.status.success(),
            "{}",
            String::from_utf8_lossy(&output.stderr)
...
#[test]
fn selected_package_obeys_download_and_dry_run() {
    let fixture = Fixture::new();
    fixture.program("npx");
...
fn explain_clean_keeps_every_target_even_with_yes() {
    ...
    let output = fixture.run(&["--dry-run", "clean", "--yes"], "true");
    assert!(
        output.status.success(),
        "{}",
        String::from_utf8_lossy(&output.stderr)
...
fn go_task_stamps_vcs_inside_a_checkout() {
    ...
    let output = fixture.run(&["run", "auditgo"], "false");
    assert!(
        output.status.success(),
        "{}",
        String::from_utf8_lossy(&output.stderr)
...
fn cli_runs_an_explicit_shell_command_through_the_host_rung() {
    let fixture = Fixture::new();
    let output = fixture.run(&["run", "/bin/sh", "-c", "printf '%s' 'a b'"], "false");
    assert!(
        output.status.success(),
        "{}",
        String::from_utf8_lossy(&output.stderr)
...
fn cli_yarn_variant_uses_the_observed_package_manager_declaration() {
    for (version, verb) in [("1.22.22", "run"), ("4.0.0", "exec")] {
        ...
        let output = fixture.run(&["run", "runner-local-variant-tool"], "false");
        assert!(
            output.status.success(),
            "{}",
            String::from_utf8_lossy(&output.stderr)
...
fn builtin_explanations_render_without_running_or_cleaning() {
    ...
    for alias in [false, true] {
        for args in [
            vec!["--dry-run", "list"],
            vec!["--dry-run", "list", "--json"],
            vec!["--dry-run", "clean", "--yes"],
            vec!["--dry-run", "-p", "list", "info"],
        ] {
...
fn cli_refuses_node_jsx_but_allows_capable_runtimes() {
    ...
    for extension in ["jsx", "tsx"] {
        ...
        for runtime in ["bun", "deno"] {
            let output = fixture.run(&["run", "--runtime", runtime, &file], "false");
            assert!(
                output.status.success(),
                "{}",
                String::from_utf8_lossy(&output.stderr)
...
fn runtime_override_handles_node_shebangs_and_preserves_shell_scripts() {
    ...
    for name in ["entry.js", "entry"] {
        ...
        let output = fixture.run(&["run", "--runtime", "bun", &token, "two words"], "false");
        assert!(
            output.status.success(),
            "{}",
            String::from_utf8_lossy(&output.stderr)
    ...
    let output = fixture.run(&["run", "--runtime", "bun", "./shell.sh"], "false");
    assert!(output.status.success());
...
fn jsx_refusal_identifies_cli_and_environment_runtime_origins() {
    ...
    let cli = fixture.run(&["run", "--runtime", "node", "view.jsx"], "false");
...
fn execute_only_source_uses_its_runtime() {
    ...
    for token in ["entry.js", "./entry.js"] {
        let output = fixture.run(&["run", token], "false");
        assert!(
            output.status.success(),
            "{}",
            String::from_utf8_lossy(&output.stderr)
...
#[test]
fn builtin_arguments_preserve_parent_download_policy() {
    let fixture = Fixture::new();
    for alias in [false, true] {
        let output = builtin_command(&fixture, alias, &["--no-download", "install", "--no-tools"])
            .env("RUNNER_DOWNLOAD", "true")
            .output()
            .unwrap();
        assert!(!output.status.success());
        assert!(String::from_utf8_lossy(&output.stderr).contains("refused"));
...
fn builtin_install_applies_its_script_flags() {
    ...
    for alias in [false, true] {
        let output = builtin_command(
            &fixture,
            alias,
            &["--download", "install", "--no-tools", "--no-scripts"],
        )
        .output()
        .unwrap();
        assert!(
...
fn removed_internal_flag_is_rejected() {
    let fixture = Fixture::new();
    let output = fixture.run(&["--internal-runner-builtin", "list"], "false");
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr).contains("unexpected argument"));
    fixture.assert_not_executed();
}
...
fn yarn_install_preview_and_execution_share_the_registry_variant() {
    for (version, frozen, verb, script_env) in [
        ...
        let explained = fixture.run(&["--dry-run", "run", "missing-widget"], "false");
        assert!(
            String::from_utf8_lossy(&explained.stderr).contains(&format!("\"yarn\", \"{verb}\""))
        );
        let why = fixture.run(&["why", "build", "--json"], "false");
        assert!(
            why.status.success(),
            "{}",
...
"#,
    ...
    let output = fixture.run(&["install"], "true");
    assert!(
        output.status.success(),
        "{}",
        String::from_utf8_lossy(&output.stderr)
...
fn install_refuses_broken_yarn_observations_before_spawning() {
    ...
    fixture.program("yarn");
    let output = fixture.run(&["install", "--no-tools", "--frozen"], "true");
    assert!(!output.status.success());
...
fn observed_runtime_keeps_precedence_for_package_exec() {
    ...
    fixture.program("bun");
    let output = fixture.run(
        &["--runtime", "bun", "--dry-run", "run", "audit-missing"],
        "true",
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
...
fn qualified_task_environment_overrides_the_bare_task() {
    ...
    let output = fixture.run(&["run", "root:package.json#first"], "false");
    assert!(
        output.status.success(),
        "{}",
        String::from_utf8_lossy(&output.stderr)
...
fn unreadable_installed_package_never_becomes_a_fetch_plan() {
    ...
    let output = fixture.run(
        &["--dry-run", "run", "--package", "audit-dep", "audit-dep"],
        "true",
    );
    assert!(!output.status.success());
    assert!(!String::from_utf8_lossy(&output.stderr).contains("Network"));
...
fn a_forced_install_without_files_or_executables_has_no_plan() {
    ...
    let output = fixture.run(
        &["--pm", "npm", "--dry-run", "install", "--no-tools"],
        "true",
    );
    assert!(!output.status.success());
...
#[test]
fn a_disabled_tools_step_never_runs_mise() {
    let fixture = Fixture::new();
    ...
    let logged = std::fs::read_to_string(fixture.0.join("executed")).unwrap();
    assert!(!logged.contains("mise"), "{logged}");

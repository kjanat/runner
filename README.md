...
COPY --from=ghcr.io/kjanat/runner:0.26.0 /run /usr/local/bin/run
COPY --from=ghcr.io/kjanat/runner:0.26.0 /runner /usr/local/bin/runner
```
...
```sh
docker run --rm kjanat/runner:0.26.0 --version

...
- A make target forwards only `NAME=value` assignments, which the recipe reads
  as `$(NAME)`. Any other word is refused before make runs, naming the word
  and the two ways to pass it, instead of landing in make's own option parser
  or becoming a bogus goal (`--help` printed make's help; `-- --help` ran the
  recipe and then failed on a missing target). `run --help` says so (#128).

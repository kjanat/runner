#!/usr/bin/env bash
# Subcommands for aur-release.yml. One script per workflow; dispatch at the bottom.
...
cmd_prepare() {
	...
	# would otherwise be at the mercy of `&`, `/`, `\`, and newlines.
	# Keeping the alphabet to [0-9A-Za-z.-] guarantees the substitution is
...
case "${1-}" in
	prepare)
		...
		;;
	*)
		echo "usage: ${0##*/} prepare <pkgname> <version>" >&2
		exit 2
		;;
esac

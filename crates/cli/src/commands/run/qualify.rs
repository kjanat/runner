...
pub(crate) fn member_ambiguity_error(
    ...
    let names: Vec<String> = members.iter().map(|member| member.label.clone()).collect();
    member_ambiguity_message(task_name, &names)
}

/// [`member_ambiguity_error`] from member names alone.
pub(crate) fn member_ambiguity_message(task_name: &str, names: &[String]) -> anyhow::Error {
